...
"""
Uniswap v3 PPO Training - Paper-Based Approach
...
Key features:
1. Formula-based fee calculation (Equations 5-6 from paper)
2. LVR (Loss-Versus-Rebalancing) penalty in reward (Equations 15-17)
3. Hourly resampled price data (not swap-by-swap)
4. State space: price, tick, width, liquidity, volatility, moving averages
...
class UniswapV3PaperEnv(gym.Env):
    ...
    def __init__(
        ...
        initial_x: float = 2.0,  # Initial amount of risky token (ETH)
        gas_cost_usd: float = 0.50,  # Arbitrum L2 gas cost (~$0.50 per rebalance)
        action_ticks: List[int] = [0, 50, 100, 200, 500],  # Wider tick options: ~0.5%, 1%, 2%, 5% ranges
        mode: str = "train",
    ...
    def _compute_lvr(self, sigma: float, price: float, L: float) -> float:
        """
        Compute instantaneous LVR (Loss-Versus-Rebalancing) from Equation 16.
        ℓ_t(σ, p_t) = L * σ² / 4 * √p_t
        """
        if L <= 0 or price <= 0:
            return 0.0
        return (L * sigma * sigma / 4.0) * math.sqrt(price)
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        tick_normalized = tick / 10000.0  # Normalize tick to reasonable range
        width_normalized = self.lp_width_ticks / 100.0  # Width in units of 100 ticks
        liquidity_normalized = self.liquidity / 1e6 if self.liquidity > 0 else 0.0
        volatility_normalized = min(volatility * 100, 1.0)  # Scale up small volatilities
    ...
    def step(self, action: int):
        ...
        if tick_width == 0:
            ...
            if self.has_lp and self.liquidity > 0:
                ...
                lvr = self._compute_lvr(sigma, price_t0, self.liquidity)
                reward = fee - lvr
        else:
            ...
            y_amount = self.initial_x * price_t0
            new_L = self._compute_liquidity(price_t0, new_lower_price, new_upper_price, self.initial_x, y_amount)
            
            self.initial_value_usd = self.initial_x * price_t0 + y_amount
            ...
            lvr = self._compute_lvr(sigma, price_t0, new_L)
            reward = fee - lvr
        ...
        if self.has_lp and self.lp_lower_tick is not None:
            ...
            if not in_range:
                ...
                position_value = self.initial_value_usd if hasattr(self, 'initial_value_usd') else self.initial_x * price_t1 * 2
...
def make_env_fn(
    ...
    action_ticks: List[int] = [0, 50, 100, 200, 500],
    mode: str = "train",
):
    def _init():
...
def train_paper_method(
    ...
    train_fn = make_env_fn(hourly_data, action_ticks=action_ticks, mode="train")
    eval_fn = make_env_fn(hourly_data, action_ticks=action_ticks, mode="eval")
    ...
    model = PPO(
        ...
        gae_lambda=0.95,
        policy_kwargs=dict(
            net_arch=[64, 64],  # Paper uses [4,2] to [10,2] - we use slightly larger
        ),
