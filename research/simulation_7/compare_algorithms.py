...
def evaluate_lstm_dqn(data_dir: str, n_episodes: int = 10, seq_len: int = 24, device: str = "cpu"):
    ...
    model_path = "comparison_lstm_dqn_best.pth"
    if not os.path.exists(model_path):
        model_path = "comparison_lstm_dqn_final.pth"
...
def evaluate_dqn(data_dir: str, n_episodes: int = 10, device: str = "cpu"):
    ...
    results = evaluate_on_test(
        data_dir=data_dir,
        model_path="comparison_dqn_best.pth",
        n_episodes=n_episodes,
        device=device,
    )
    return results
...
def visualize_ppo_decisions(data_dir: str, model_path: str = "comparison_ppo.zip"):
...
def visualize_dqn_decisions(data_dir: str, model_path: str = "comparison_dqn_best.pth", device: str = "cpu"):
...
def visualize_lstm_decisions(data_dir: str, model_path: str = "comparison_lstm_dqn_best.pth", 
...
def plot_lstm_decisions(lstm_data: Dict, save_path: str = "test_lstm_decisions.png"):
...
def plot_decisions(ppo_data: Dict, dqn_data: Dict, save_path: str = "test_decisions.png"):
...
def main():
    ...
    if args.lstm_only:
        ...
        # Save results
        with open("comparison_results_lstm.json", "w") as f:
            json.dump(results, f, indent=2, default=str)
        print(f"\n💾 Results saved to comparison_results_lstm.json")
        return
    ...
    if not args.no_plot and not args.baselines_only:
        ...
        try:
            ppo_data = visualize_ppo_decisions(args.data_dir, "comparison_ppo.zip")
            dqn_data = visualize_dqn_decisions(args.data_dir, "comparison_dqn_best.pth", args.device)
            plot_path = plot_decisions(ppo_data, dqn_data, "test_decisions.png")
