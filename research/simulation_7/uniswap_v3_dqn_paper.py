...
"""
Uniswap v3 Dueling DDQN Training - Paper-Based Approach
...
5. Formula-based fee calculation
"""
...
class UniswapV3DQNEnv(gym.Env):
    ...
    def __init__(
        ...
        # =====================================================================
        # ENHANCED ACTION SPACE: Width + Offset combinations
        # =====================================================================
        # Widths: Different LP range sizes (in tick_spacing units)
        self.widths = [5, 10, 25, 50]  # ~0.5%, 1%, 2.5%, 5% ranges
        
        # Offsets: Position center relative to current price
        # Offset in multiples of (width * tick_spacing / 2)
        # -2 = position below (bearish), 0 = centered, +2 = position above (bullish)
        self.offsets = [-2, -1, 0, 1, 2]
        
        # Build action mapping: action_id -> (width, offset)
        # Action 0 = HOLD
        self.action_map = {0: (None, None)}  # HOLD
        action_id = 1
        for width in self.widths:
            for offset in self.offsets:
                self.action_map[action_id] = (width, offset)
                action_id += 1
        ...
        # State space: technical features (22) + position info (6) = 28
        # Added: position_offset (where LP is relative to price)
        # Tech features: 22 original + 9 trend features = 31
        n_tech_features = 31
        self.state_dim = n_tech_features + 6  # features + (cash_ratio, width, in_range, position_value_ratio, position_offset, price_momentum)
        ...
        # Discrete action space with width + offset combinations
        # Total: 1 (HOLD) + 4 widths * 5 offsets = 21 actions
        self.action_space = spaces.Discrete(self.n_actions)
        
        self._reset_state()
        
        # Print action space info on first creation
        print(f"🎮 Enhanced Action Space: {self.n_actions} actions")
        print(f"   - HOLD (action=0)")
        print(f"   - Widths: {self.widths} ticks")
        print(f"   - Offsets: {self.offsets} (in half-width units)")
        print(f"   - Example: action=3 → width={self.action_map[3][0]}, offset={self.action_map[3][1]}")
    ...
    def _reset_state(self):
        ...
        self.liquidity = 0.0
        self.position_offset_ticks = 0  # Track offset for state observation
    ...
    def _compute_lvr(self, price_t0: float, price_t1: float) -> float:
        """
        Compute LVR (Loss-Versus-Rebalancing) for one step.
        LVR = V(p_{t+1}) - V(p_t) - x(p_t) × (p_{t+1} - p_t)
        ...
        # x(p_t) = amount of token X at price p_t
        lower_tick = self.position_center_tick - self.position_width * self.tick_spacing
        upper_tick = self.position_center_tick + self.position_width * self.tick_spacing
        p_lower = tick_to_price(lower_tick)
        p_upper = tick_to_price(upper_tick)
        ...
        # LVR = change in position value - rebalancing profit
        lvr = (V_t1 - V_t0) - x_t0 * (price_t1 - price_t0)
        return lvr  # This is <= 0
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        # Position features (6) - enhanced with offset and momentum
        ...
        # Position offset: where is LP center relative to current price
        # Normalized by max_width * tick_spacing
        if self.has_position:
            current_tick = price_to_tick(price)
            offset_ticks = self.position_center_tick - current_tick
            position_offset = offset_ticks / (self.max_width * self.tick_spacing)
        else:
            position_offset = 0.0
        ...
        position_features = np.array([
            ...
            position_offset, price_momentum
        ], dtype=np.float32)
    ...
    def step(self, action: int):
        ...
        width, offset = self.action_map.get(action, (None, None))
        ...
        if width is None:
            ...
            # Reallocate with width and offset (always costs gas)
            # Agent can express directional views via offset:
            #   offset < 0: position below current price (bearish)
            #   offset = 0: centered at current price
            #   offset > 0: position above current price (bullish)
            ...
            # Gas cost - agent learns when it's worth paying
            self.cash -= self.gas_cost
            reward -= self.gas_cost
            
            # Calculate center tick with offset
            tick_t0 = price_to_tick(price_t0)
            base_center_tick = (tick_t0 // self.tick_spacing) * self.tick_spacing
            
            # Offset is in half-width units, converted to ticks
            offset_ticks = offset * (width * self.tick_spacing // 2)
            center_tick = base_center_tick + offset_ticks
            
            # Store offset for state observation
            self.position_offset_ticks = offset_ticks
            ...
            # Need to handle the case where current price is outside position range
        ...
        # Reward shaping for position management
        if self.has_position:
            if in_range:
                # In-range bonus: reward for correct directional positioning
                # This teaches: "Predicting direction correctly = good"
                in_range_bonus = 0.05  # Small bonus for staying in range
                reward += in_range_bonus
            else:
                ...
                opportunity_rate = 0.00006  # 5% APY / 365 / 24
...
def train_dqn(
    ...
    """
    Train Dueling DDQN following Zhang et al. (2023).
    """
    print("=" * 60)
