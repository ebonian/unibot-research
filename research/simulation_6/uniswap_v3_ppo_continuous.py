...
class UniswapV3ContinuousEnv(gym.Env):
    ...
    def __init__(
        ...
        # Load pool data - dynamically detect data files by pattern
        ...
        # Auto-detect data files by pattern matching (supports any date range)
        ...
        if not all([slot0_files, swaps_files, mints_files, burns_files, ethusdt_files]):
            raise FileNotFoundError(f"Missing required data files in {data_dir}. Expected: slot0_*, swaps_*, mints_*, burns_*, ETHUSDT_hourly_data_*")
        
        slot0 = pd.read_csv(slot0_files[0])
        swaps = pd.read_csv(swaps_files[0])
        mints = pd.read_csv(mints_files[0])
        burns = pd.read_csv(burns_files[0])
        ethusdt = pd.read_csv(ethusdt_files[0])
...
if __name__ == "__main__":
    # Use training_data folder in the same directory as this script
