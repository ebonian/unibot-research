...
class UniswapV3FeeSimulator:
    ...
    def __init__(self,
        ...
        # LRU cache for simulate() - same (window, range, capital) returns cached result (exact, no accuracy loss)
        self._simulate_cache: OrderedDict = OrderedDict()
        self._simulate_cache_max = 20_000
    ...
    def simulate(self,
        ...
        if total_usd is not None and not validate and not use_swap_liquidity:
            cache_key = (start, end, round(price_lower, 10), round(price_upper, 10), round(total_usd, 4))
            if cache_key in self._simulate_cache:
                self._simulate_cache.move_to_end(cache_key)
                return self._simulate_cache[cache_key]
        ...
        if cache_key is not None:
            ...
            if len(self._simulate_cache) > self._simulate_cache_max:
