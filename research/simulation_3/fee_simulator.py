    ...
    def simulate(self,
        ...
        return SimulationSummaryOut(
            ...
            start_principal_tokens=TokenAmountsOut(
                ...
                token1_usd=float(start_principal_token1_usd),
                total_usd=(start_principal_token0_usd if start_principal_token0_usd is not None else 0.0) + float(start_principal_token1_usd),
            ),
            end_tokens=TokenAmountsOut(
                token0_amount=float(end_tot0_eth),
                token1_amount=float(end_tot1_usd),
                token0_usd=end_token0_usd,
                token1_usd=float(end_token1_usd),
                total_usd=(end_token0_usd if end_token0_usd is not None else 0.0) + float(end_token1_usd),
            ),
            end_principal_tokens=TokenAmountsOut(
                token0_amount=float(end_principal0_eth_units),
                token1_amount=float(end_principal1_token1),
                token0_usd=end_principal_token0_usd,
                token1_usd=float(end_principal_token1_usd),
                total_usd=(end_principal_token0_usd if end_principal_token0_usd is not None else 0.0) + float(end_principal_token1_usd),
            ),
